...
> 版本 v1.14（**定稿**，可进入实现）· 最后更新 2026-09-18
...
> - v1.14：**修复粘贴宽内容把实时预览挤出视口**——v1.5 已给内层 `.editor-wrap/.preview-wrap/.editor/.preview` 补 `min-width:0`，但作为 `.body` 行向 flex 项的 `.split` 容器本身仍缺该声明，导致预览区中**不可断行的宽内容**（长 URL / `pre` 代码块 / 宽表格）抬高其 min-content 宽度、沿 flex 链把 `.split` 撑破并整页横向溢出。给 `.split` 补 `min-width:0` 后，宽内容改在预览区内横向滚动，不再推移布局（见 §4.11）。
...
- **文章主题 ≠ 界面外观（各自独立持久化）；微信深色预览随界面外观深浅同步（v1.12）**：文章主题决定预览/复制输出；界面外观为**全局**设置，三选 **浅色 / 深色 / 跟随系统**——「跟随系统」经 `matchMedia("(prefers-color-scheme: dark)")` 解析出**实际生效深浅** `resolvedTheme` 并随 OS 实时更新；切到深色时整个软件 chrome（标题栏/菜单/侧栏/工具栏/弹窗等）同步变暗，相关配色经 CSS 变量统一调整。**「微信深色预览」开关**（v1.9 引入）**不再与外观完全独立**：每当生效深浅变化（手动切换或「跟随系统」下 OS 变化），`wx_dark` 自动同步为 `resolvedTheme==='dark'`，使预览区深浅始终随主界面一致；开启时对预览 HTML 的内联颜色做 **mp-darkmode 风格 HSL 映射**（替换 v1.3 的整块 CSS `invert` 反相），预览容器加深色底。按钮保留供**会话内手动覆盖**，下次深浅变化会重新同步，启动时不再独立恢复 `wx_dark`（改由 `resolvedTheme` 派生）。**该映射只作用于前端预览展示，复制/导出始终由 Rust 从 md 重新渲染、输出内联样式 HTML 不变**（详见 §4.14）。
...
- 「视图 ▸ 外观」为**全局浅色 / 深色 / 跟随系统**三选（原「编辑器外观」升级，见 §4.4）；「跟随系统」经 `prefers-color-scheme` 随 OS 深浅实时生效。
...
- **随界面外观深浅同步（v1.12 修订 v1.9「完全独立」）**：预览区标题栏保留「微信深色预览」切换按钮（日/月图标）。每当**生效深浅** `resolvedTheme` 变化（手动切换浅/深/跟随系统，或「跟随系统」下 OS 深浅改变），`wx_dark` 自动同步为 `resolvedTheme==='dark'`，使预览区深浅始终随主界面一致。按钮仍可**会话内手动覆盖**（`Config.wx_dark` 持久化该覆盖值），但下次深浅变化会重新同步；**启动时不再独立恢复** `wx_dark`，改由 `resolvedTheme` 派生。
...
**v1.9 追加决策（用户确认）**
40. **微信深色预览 = 独立开关 + 前端 HSL 颜色映射**（对比报告 §3.1 进阶版）：与界面外观**解耦**为独立持久化偏好（`Config.wx_dark`），首次启动默认跟随外观以保留 v1.3 直觉，此后独立。开启时前端 `wxDark.ts` 用 `DOMParser` 逐元素映射内联颜色（前景深→翻亮/浅→保持，背景浅→压暗/深→再压暗不翻亮），替换 v1.3 整块 `invert`。**核心约束：只作用于预览展示——复制/导出始终由 Rust 从 md 重新渲染，`toWxDark` 不在输出路径上，故输出 HTML 100% 不变**（见 §4.14）。**〔v1.12 修订〕**「完全独立/解耦」部分作废：`wx_dark` 改为随生效深浅 `resolvedTheme` 自动同步，开关仅保留为会话内手动覆盖（见 #43）。
...
**v1.12 追加决策（用户确认）**
43. **界面外观三选（浅/深/跟随系统）+ 微信深色预览随生效深浅同步**：① `editor_theme` 扩为 `light/dark/system`；`system` 时用 `window.matchMedia("(prefers-color-scheme: dark)")`（WebView2 跟随 OS）解析出**生效深浅** `resolvedTheme`，并监听 `change` 事件实时更新——`html.dark` 类与编辑器 `appearance` 均用 `resolvedTheme`，视图菜单单选仍反映 `light/dark/system` 偏好本身。② 每次 `resolvedTheme` 变化（手动切换或「跟随系统」下 OS 变化）用一个 `useEffect` 把 `wx_dark` 同步为 `resolvedTheme==='dark'`，保证预览区深浅随主界面一致；「微信深色预览」按钮保留为**会话内手动覆盖**，启动时不再独立恢复 `wx_dark`（改由 `resolvedTheme` 派生）。**安全约束不变**：HSL 映射只作用于前端预览，复制/导出由 Rust 从 md 重新渲染、输出 HTML 100% 不变（见 §4.4/§4.10/§4.14）。
